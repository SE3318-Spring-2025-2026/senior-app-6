...
export function useApiClient() {
	...
	async function updateDeliverable(
		...
		return apiCall<Deliverable>(
			`/coordinator/deliverables/${encodeURIComponent(deliverableId)}`,
			"PATCH",
			data,
			token
		);
	...
	async function createSprint(sprint: CreateSprintRequest, token?: string): Promise<Sprint> {
	...
	async function updateSprintTarget(
		...
		return apiCall<void>(
			`/coordinator/sprints/${encodeURIComponent(sprintId)}/target`,
			"PATCH",
