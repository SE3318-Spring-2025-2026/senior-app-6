...
sequenceDiagram
    ...
    alt action == ASSIGN and advisorId is null
        CoordinatorController-->>Coordinator: 400 {"message": "advisorId is required for ASSIGN action"}
    end
    ...
    alt group not found
        ...
        CoordinatorController-->>Coordinator: 404 {"message": "Group not found"}
    end
    ...
    alt advisor not found or not a Professor
        ...
        CoordinatorController-->>Coordinator: 404 {"message": "Advisor not found"}
    end

    alt group.advisorId == advisorId (already assigned)
        AdvisorService-->>CoordinatorController: throw BusinessRuleException
        CoordinatorController-->>Coordinator: 400 {"message": "Group already has this advisor assigned"}
    end
...
sequenceDiagram
    ...
    alt group.advisorId is null
        AdvisorService-->>CoordinatorController: throw BusinessRuleException
        CoordinatorController-->>Coordinator: 400 {"message": "Group has no advisor to remove"}
    end
