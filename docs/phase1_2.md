...
| 3 | Professor logs in for the first time and is required to change their password before proceeding. | Frontend – Login Page / Backend | `email`, `temporaryPassword`, `newPassword` |
...
> Extracted directly from the Project Definition document for reference.

```
─── Team Evaluation ────────────────────────────────────────────────────────────

  For each Deliverable:
    ScrumScalar  = AVG(Point A grades for contributing sprints) ÷ 100
    ReviewScalar = AVG(Point B grades for contributing sprints) ÷ 100
    DS           = AVG(ScrumScalar, ReviewScalar)
    ScaledGrade  = BaseDeliverableGrade × DS

  WeightedTotal = Σ (ScaledGrade × DeliverableWeight)

─── Individual Evaluation ──────────────────────────────────────────────────────

  Cᵢ = completedStoryPoints ÷ targetStoryPoints
  Gᵢ = WeightedTotal × Cᵢ

─── Example (from Project Definition) ─────────────────────────────────────────

  Sprint Grades:
    Point A (Scrum):  Sprint1=B(80), Sprint2=A(100), Sprint3=A(100)
    Point B (Review): Sprint1=C(60), Sprint2=B(80),  Sprint3=A(100)

  Sprint-to-Deliverable Mapping:
    Proposal     ← Sprint 1
    SoW          ← Sprint 1, 2
    Demonstration← Sprint 1, 2, 3

  Scalars:
    Proposal  → ScrumScalar=0.7, ReviewScalar=0.6  → DS=0.65
    SoW       → ScrumScalar=0.9, ReviewScalar=0.7  → DS=0.80
    Demonstration → ScrumScalar=0.93, ReviewScalar=0.8 → DS=0.865

  Base Deliverable Grades:
    Proposal=90, SoW=94, Demonstration=92

  Scaled Grades:
    Proposal:      90 × 0.65 = 58.50
    SoW:           94 × 0.80 = 75.20
    Demonstration: 92 × 0.865 = 79.58

  Deliverable Weights (Coordinator-configured):
    Documents 50%: Proposal=15%, SoW=35%
    Demonstration 50%

  WeightedTotal = (58.50×0.15) + (75.20×0.35) + (79.58×0.50)
                = 8.77 + 26.32 + 39.79 = 74.88

  Individual (Student completed 9/10 story points → Cᵢ = 0.9):
    Gᵢ = WeightedTotal × Cᵢ = 79.58 × 0.9 = 71.622
    (As stated in the Project Definition: "The student will get 0.9 of the overall grade that team takes, which is 71.622")

```

---

## Business Process Diagram: Deliverable Submission, Review, Grading & Calculation (Process 6 & 7)

```mermaid
graph TD

    subgraph P6_Submission["Process 6: Phase 6A - Submission"]
        A1([Team Leader: Opens WYSIWYG Markdown Editor])
        A2{Group assigned<br/>to a Committee?}
        A3{Deadline<br/>Passed?}
        A4[Author Content in WYSIWYG Editor<br/>Markdown, images, rubric-linked sections]
        A5[Timestamp and persist to Database]
        A6N[Notify All Committee Members<br/>Advisor and Jury]
        ERR1([ERROR: Cannot Submit<br/>Committee assignment required])
        ERR2([ERROR: Submission Rejected<br/>Deadline has passed])
    end

    subgraph P6_Review["Process 6: Phase 6B - Review"]
        B1[Committee Member Opens Submission<br/>with linked Evaluation Rubric]
        B2[Leave Inline Comments and Feedback]
        B3{Changes<br/>Requested?}
        B4[Group Revises Content<br/>via WYSIWYG Editor]
        B5[Persist revised submission]
    end

    subgraph P7_Grading["Process 7: Phase 7A - Grading"]
        C1["Grade Each Rubric Criterion<br/>Binary: S=100 or F=0<br/>Soft: A=100 B=80 C=60 D=50 F=0"]
        C2[Persist criterion grades]
        C3[Aggregate All Committee Scores<br/>Base Deliverable Grade B]
    end

    subgraph P7_Calculation["Process 7: Phase 7B - Scalar Calculation"]
        D1[Identify Contributing Sprints<br/>per Deliverable]
        D2[Compute Scrum Scalar<br/>ScrumScalar = AVG of Point A grades / 100]
        D3[Compute Review Scalar<br/>ReviewScalar = AVG of Point B grades / 100]
        D4[Compute Deliverable Scalar<br/>DS = AVG of ScrumScalar and ReviewScalar]
        D5[Compute Scaled Deliverable Grade<br/>ScaledGrade = B x DS]
        D6N[Compute Weighted Total<br/>WeightedTotal = Sum of ScaledGrade x DeliverableWeight]
        D7[Fetch Individual Completion Ratio<br/>Ci = completedStoryPoints / targetStoryPoints]
        D8[Compute Individual Final Grade<br/>Gi = WeightedTotal x Ci]
        D9[(Persist Final Grades to Database)]
        D10([Advisor Dashboard Updated<br/>Final Grades Visible])
    end
