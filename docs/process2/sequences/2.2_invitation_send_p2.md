...
sequenceDiagram
    ...
    alt q.length < 3
        StudentSearchController-->>Student: 400 {"message": "Query must be at least 3 characters"}
    end
...
sequenceDiagram
    ...
    alt requester is not TEAM_LEADER of this group
        ...
        GroupController-->>Student: 403 {"message": "Only the Team Leader can send invitations"}
    end
    ...
    alt group.status == DISBANDED
        ...
        GroupController-->>Student: 400 {"message": "This group has been disbanded"}
    end

    alt group.status == TOOLS_BOUND or ADVISOR_ASSIGNED
        GroupService-->>GroupController: throw BusinessRuleException
        GroupController-->>Student: 400 {"message": "Group roster is locked after tool binding"}
    end
    ...
    alt memberCount + pendingCount >= maxTeamSize
        GroupService-->>GroupController: throw BusinessRuleException
        GroupController-->>Student: 400 {"message": "Group has reached maximum team size"}
    end
    ...
    alt student not found
        ...
        GroupController-->>Student: 400 {"message": "Student '{id}' is not registered in the system"}
    end
    ...
    alt target already in a group
        ...
        GroupController-->>Student: 400 {"message": "Student '{id}' is already a member of a group"}
    end
    ...
    alt pending invitation already exists
        ...
        GroupController-->>Student: 409 {"message": "A pending invitation already exists for this student"}
    end
...
sequenceDiagram
    ...
    alt not TEAM_LEADER
        GroupService-->>GroupController: throw ForbiddenException
        GroupController-->>Student: 403 {"message": "Only the Team Leader can view group invitations"}
    end
...
sequenceDiagram
    ...
    alt not found
        ...
        InvitationController-->>Student: 404 {"message": "Invitation not found"}
    end
    ...
    alt requester is not TEAM_LEADER of the inviting group
        ...
        InvitationController-->>Student: 403 {"message": "Only the Team Leader can cancel invitations"}
    end

    alt invitation.status != PENDING
        GroupService-->>InvitationController: throw InvitationNotPendingException
        InvitationController-->>Student: 400 {"message": "Invitation is no longer pending"}
    end
