...
> ⚠️ `termId` defaults to the active term via `TermConfigService.getActiveTermId()` when omitted. Supports `"ALL"` to return groups across all terms.
...
sequenceDiagram
    ...
    Coordinator->>CoordinatorGroupController: GET /api/coordinator/groups?termId={optional}<br/>Authorization: Bearer <staff_jwt>

    Note over CoordinatorGroupController: Role=COORDINATOR enforced by SecurityConfig — no manual check

    CoordinatorGroupController->>GroupService: getGroupSummaries(termId)

    alt termId is null or blank
        GroupService->>TermConfigService: getActiveTermId()
        TermConfigService-->>GroupService: activeTermId
        GroupService->>ProjectGroupRepository: findByTermId(activeTermId)
    else termId == "ALL"
        GroupService->>ProjectGroupRepository: findAll()
    else specific termId
...
sequenceDiagram
    ...
    alt action == ADD
        ...
        alt group.status == DISBANDED
            ...
            CoordinatorGroupController-->>Coordinator: 400 {"error": "Cannot add a student to a disbanded group"}
        end
        ...
        GroupService->>GroupInvitationRepository: autoDenyAllPendingByInviteeId(student.id)
        GroupInvitationRepository-->>GroupService: updatedCount

        GroupService->>AuditLogService: record(coordinatorUUID, STAFF, MEMBER_ADDED, GROUP, SUCCESS)
        ...
        alt membership not found
            GroupService-->>CoordinatorGroupController: throw GroupNotFoundException
            CoordinatorGroupController-->>Coordinator: 404 {"error": "Student '{id}' is not a member of this group"}
        end
        ...
        GroupService->>AuditLogService: record(coordinatorUUID, STAFF, MEMBER_REMOVED, GROUP, SUCCESS)
    end
...
sequenceDiagram
    ...
    Note over GroupService: @Transactional — all operations atomic
    ...
    GroupService->>GroupMembershipRepository: deleteByGroupId(groupId)
    GroupMembershipRepository-->>GroupService: ok

    Note over GroupService: Auto-deny all PENDING outbound invitations from this group
    GroupService->>GroupInvitationRepository: autoDenyAllPendingByGroupId(groupId)
    GroupInvitationRepository-->>GroupService: updatedCount

    Note over GroupService: P3 cascade — auto-reject all PENDING advisor requests for this group
    GroupService->>AdvisorRequestRepository: bulkUpdateStatusByGroupId(AUTO_REJECTED, groupId)
    AdvisorRequestRepository-->>GroupService: updatedCount

    GroupService->>AuditLogService: record(coordinatorUUID, STAFF, GROUP_DISBANDED, GROUP, SUCCESS)

    GroupService-->>CoordinatorGroupController: GroupDetailResponse (status=DISBANDED, members=[])
    CoordinatorGroupController-->>Coordinator: 200 {id, groupName, termId, status:"DISBANDED", jiraBound, githubBound, members:[]}
```
